...
def Error(text):                                                                                        # prints information about the error and halts
    ...
    print("\nERROR " + text + " in line " + ln + ": '" + line + "'",flush = True) #exit(1)
...
import  random; import msvcrt, os; os.system("")

print("MIN Shell",flush = True)
block_word_list = ['if','while','else','elif','def']  
file = "" #a text buffer for name compatibility
isDef = False  #to mark whether a block is a function defining block
indent = 0  #the current indentation level
comm_list=[] #the actual command buffer

while True:
    if indent==0:
        print(">>> ",end='',flush = True)
        comm = input()
        for word in block_word_list: #increase the indentation level if key words detected
            if word in comm:
                indent+=1  
        ...
        if indent:
            continue
        file += comm + '\0'  # append this command to "file"
        bk = g.pc  #store the current pc
        while Look() != '\0': ErrorIf(not Statement(), "unexpected indent") #run until exit
        file = file[0:-(len(comm)+1)]  # pop out the command that has been executed
        g.pc=bk     #restore pc
    else:
        print("... ",indent*"->",sep='',end='',flush = True)    #if indent>0, print auto indentation mark "->""
        comm = '' #the line buffer
        while True:
            ch = msvcrt.getch()
            v_ch = ch[0]
            
            if indent and v_ch>=32 and v_ch<=126:   #check for visible characters, store and echo
                comm+=str(ch,encoding="utf-8")
                msvcrt.putch(ch)  
                continue
            
            if indent and v_ch==0x08 :
                if len(comm)!=0:     # if we have entered something and then press "Backspace", delete one character
                    comm = comm[0:-1]
                    msvcrt.putch(b'\x08')
                    msvcrt.putch(b'\x20')
                    msvcrt.putch(b'\x08')
                else:
                    indent-=1  # if we press "Backspace" without any characters in the line buffer, decrease the current indentation level
                    for i in range(2):
                        msvcrt.putch(b'\x08')
                        msvcrt.putch(b'\x20')
                        msvcrt.putch(b'\x08')
            ...
            if len(comm)==0 and v_ch==0x09: # we can press "Tab" to increase indentation level
                indent+=1
                print("->",end='',flush = True)
                continue
        ...
        msvcrt.putch(b'\x0a')
        if len(comm):
            comm_list.append(indent*"  "+comm)
            for word in block_word_list:
                if word in comm:
                    indent+=1            
            
            
        if indent==0:  # means that we exit the input of a block
            block_comm = ''
            for sentence in comm_list:
                block_comm += sentence+'\n'
            comm_list=[]
            file += block_comm + "\0" # append block content to "file"
            bk = g.pc
            if isDef:  
                isDef = False
                while Look() != '\0': DefFail = ErrorIf(not Statement(), "unexpected indent")
                if DefFail:  # pop out the content and restore pc if we get error
                    file = file[0:-(len(block_comm)+1)]
                    g.pc=bk 
                else:
                    file = file[0:-1] # for successful definition, just remove the null character from "file" and decrease pc for 1 position 
                    g.pc-=1
            else:
                # if not a def block, simply execute and then pop it out
